...
class DLL_PUBLIC TBaseSAGA : public TStoSolver<T, K> {
 ...
 public:
  ...
  void serialize(Archive &ar) {
    ar(cereal::make_nvp("StoSolver", typename cereal::base_class<TStoSolver<T, K>>(this)));
...
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(BaseSAGADouble, cereal::specialization::member_serialize)

using BaseSAGAFloat = TBaseSAGA<float, float>;
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(BaseSAGAFloat, cereal::specialization::member_serialize)

using BaseSAGAAtomicDouble = TBaseSAGA<double, std::atomic<double>>;
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(BaseSAGAAtomicDouble, cereal::specialization::member_serialize)

using BaseSAGAAtomicFloat = TBaseSAGA<float, std::atomic<float>>;
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(BaseSAGAAtomicFloat, cereal::specialization::member_serialize)
...
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(SAGADouble, cereal::specialization::member_serialize)
CEREAL_REGISTER_TYPE(SAGADouble)

using SAGAFloat = TSAGA<float>;
CEREAL_SPECIALIZE_FOR_ALL_ARCHIVES(SAGAFloat, cereal::specialization::member_serialize)
