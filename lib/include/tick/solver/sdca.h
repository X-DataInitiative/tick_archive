...
class DLL_PUBLIC TSDCA : public TStoSolver<T, K> {
 ...
 public:
  ...
  explicit TSDCA(T l_l2sq, ulong epoch_size = 0, T tol = 0., RandType rand_type = RandType::unif,
                 int seed = -1);
 ...
 public:
  ...
  BoolStrReport compare(const TSDCA<T, K> &that) {
    ...
    bool are_equal = TStoSolver<T, K>::compare(that, ss) && TICK_CMP_REPORT(ss, n_coeffs) &&
                     TICK_CMP_REPORT(ss, stored_variables_ready) && TICK_CMP_REPORT(ss, l_l2sq) &&
                     TICK_CMP_REPORT(ss, delta) && TICK_CMP_REPORT(ss, dual_vector);
