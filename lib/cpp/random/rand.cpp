...
Rand::Rand(const std::mt19937_64 &generator) : seed(0), generator(generator) {
  init_reusable_distributions();
}

void Rand::init_reusable_distributions() {
  // We can easily generate realizations of distributions with other parameters
  // from this ones. Hence we create them once and for all.
...
double Rand::gaussian() { return normal_dist(generator); }
...
ulong Rand::discrete() { return discrete_dist(generator); }
