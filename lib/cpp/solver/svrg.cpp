...
TSVRG<T, K>::TSVRG(ulong epoch_size, T tol, RandType rand_type, T step, int seed, int n_threads,
                   SVRG_VarianceReductionMethod variance_reduction, SVRG_StepType step_method)
...
template <class T, class K>
void TSVRG<T, K>::solve_sparse_proba_updates(bool use_intercept, ulong n_features) {
...
template <class T, class K>
void TSVRG<T, K>::sparse_single_thread_solver(const ulong& next_i, const ulong& n_features,
