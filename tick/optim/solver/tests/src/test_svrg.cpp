...
TEST(SVRG, test_sparse_exact_updates) {
  SArrayDoublePtr labels_ptr = get_labels();
  SArrayDouble2dPtr features_ptr = get_features();
  SSparseArrayDouble2dPtr sparse_features_ptr = get_sparse_features();
  ...
  for (ulong epoch_size = 0; epoch_size < 10; epoch_size+=2) {
    for (ulong n_epoch = 1; n_epoch < 5; n_epoch+=2) {
      SVRG svrg(epoch_size, 0, RandType::unif, model->get_lip_max() / 100, 1309);
      svrg.set_rand_max(n_samples);
      svrg.set_model(model);

      auto sparse_model = std::make_shared<ModelLinReg>(sparse_features_ptr, labels_ptr, false, 1);
      SVRG sparse_svrg(epoch_size, 0, RandType::unif, model->get_lip_max() / 100, 1309,
                       SVRG::VarianceReductionMethod::Last, SVRG::DelayedUpdatesMethod::Exact);
      sparse_svrg.set_rand_max(n_samples);
      sparse_svrg.set_model(sparse_model);

      ArrayDouble out_iterate(n_features);
      ArrayDouble sparse_out_iterate(n_features);
      ...
      svrg.get_iterate(out_iterate);
      sparse_svrg.get_iterate(sparse_out_iterate);

      sparse_out_iterate.mult_incr(out_iterate, -1);
      EXPECT_LE(sparse_out_iterate.norm_sq() / n_features, 1e-10);
    }
  }
}
